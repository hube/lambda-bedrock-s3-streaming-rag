...
describe.skipIf(!dockerAvailable)(
  "handler integration (LocalStack)",
  () => {
    ...
    beforeAll(async () => {
      ...
      await eb.send(
        new CreateEventBusCommand({ Name: EVENT_BUS }),
      );
    ...
    it("D.4: non-PDF key returns 200 without writing to LanceDB", async () => {
      ...
      const result = await handler(makeEvent("user1/groupA/image.png", UPLOAD_BUCKET));
      expect(result.statusCode).toBe(200);
